...
function Header({ onLogin }) {
  ...
  return (
    <header className={`container header ${isColoredHeader ? 'header_type_color' : ''}`}>
      ...
      {
        <a className={`header__btn-enter ${isColoredHeader ? 'header__btn-enter_type_color' : ''}`} href='https://dpo.emiit.ru/'>вход</a>

        //<button className={`header__btn-enter ${isColoredHeader ? 'header__btn-enter_type_color' : ''}`} onClick={onLogin} type='button'>вход</button>
      }
